...
function renderPriceLevel(price: string, c: AppColorScheme) {
  const count = (price.match(/\$/g) || []).length;
  return (
    <View style={{ flexDirection: "row" }}>
      {Array.from({ length: 4 }, (_, i) => (
        <Text key={i} style={{ fontSize: 13, fontWeight: "bold", color: i < count ? c.accent : c.border }}>
          $
        </Text>
      ))}
    </View>
...
function renderStars(rating: number, c: AppColorScheme) {
  const full = Math.floor(rating);
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 2 }}>
      {Array.from({ length: 5 }, (_, i) => (
        <Ionicons key={i} name={i < full ? "star" : "star-outline"} size={12} color={i < full ? c.accent : c.border} />
      ))}
      <Text style={{ fontSize: 12, color: c.textPrimary, marginLeft: 4 }}>{rating}</Text>
    </View>
  );
}
...
export default function ActivitiesScreen() {
  ...
  useEffect(() => {
    ...
    if (!isPaid) { setLoadingMatches(false); return; }
    getMatches()
      .then((profiles) => {
    ...
    return (
      <View style={styles.paywallContainer}>
        ...
        <Text style={styles.paywallSubtitle}>
          This hive is exclusive to paid members.{"\n"}We're sorry — the Matches tab is reserved for those who've joined the colony.
        </Text>
        ...
        <TouchableOpacity style={styles.paywallButton} onPress={() => router.push("/(tabs)/profile")}>
        ...
        {loadingMatches ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.matchesScroll}>
            {matchIds.map((id) => {
              const m = matches[id];
              return (
                <TouchableOpacity key={id} style={styles.matchBubble} onPress={() => fetchActivities(id)}>
                  <View style={[styles.bubbleAvatar, { backgroundColor: m.color }, selectedId === id && styles.bubbleAvatarActive]}>
                ...
                <View key={idx} style={styles.activityCard}>
                  ...
                  <View style={styles.activityInfo}>
                    <View style={styles.activityTop}>
                      ...
                      {renderPriceLevel(activity.estimated_cost_per_person, colors)}
                    </View>
                    <Text style={styles.activityCategory}>{activity.category}</Text>
                    <View style={styles.activityDetails}>
                      {renderStars(activity.rating, colors)}
...
function makeStyles(c: AppColorScheme) {
  return StyleSheet.create({
    ...
    matchesTitleText: { fontSize: 15, fontWeight: "600", color: c.textPrimary },
    noMatchesText: { fontSize: 13, color: c.textSecondary, marginTop: 8 },
    matchesScroll: { flexDirection: "row" },
    matchBubble: { alignItems: "center", marginRight: 16 },
    bubbleAvatar: { width: 52, height: 52, borderRadius: 26, alignItems: "center", justifyContent: "center", marginBottom: 4 },
    bubbleAvatarActive: { borderWidth: 3, borderColor: c.accent },
    bubbleInitials: { fontSize: 18, fontWeight: "bold", color: "#fff" },
    bubbleName: { fontSize: 12, color: c.textSecondary },
    selectedMatchHeader: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: c.accentLight, padding: 10, borderRadius: 10, marginBottom: 8 },
    selectedMatchText: { fontSize: 13, color: c.textPrimary },
    sharedInterests: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 12 },
    interestChip: { backgroundColor: c.card, borderRadius: 12, borderWidth: 1, borderColor: c.border, paddingHorizontal: 10, paddingVertical: 4 },
    interestChipText: { fontSize: 12, color: c.textPrimary },
    noActivities: { alignItems: "center", paddingVertical: 24 },
    noActivitiesIcon: { fontSize: 32, marginBottom: 8 },
    noActivitiesText: { fontSize: 14, color: c.textSecondary },
    activitiesList: { gap: 12 },
    activityCard: { backgroundColor: c.card, borderRadius: 16, padding: 14, shadowColor: c.shadow, shadowOpacity: 0.05, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, alignItems: "center" },
    ...
    activityName: { fontSize: 15, fontWeight: "bold", color: c.textPrimary, flex: 1, marginRight: 8 },
    activityCategory: { fontSize: 12, color: c.textSecondary, marginBottom: 6 },
    ...
    activityReason: { flexDirection: "row", alignItems: "flex-start", gap: 4, backgroundColor: c.accentLight, borderRadius: 8, padding: 8, marginBottom: 8 },
    reasonText: { fontSize: 12, color: c.accentText, flex: 1 },
    activityAddress: { flexDirection: "row", alignItems: "center", gap: 3, marginBottom: 10 },
    addressText: { fontSize: 12, color: c.textSecondary },
    paywallContainer: { flex: 1, backgroundColor: c.accentLight, alignItems: "center", justifyContent: "center", paddingHorizontal: 32, paddingVertical: 48 },
    paywallHoneycomb: { fontSize: 64, marginBottom: -16 },
    paywallBee: { fontSize: 48, marginBottom: 20 },
    paywallTitle: { fontSize: 26, fontWeight: "800", color: c.accentText, marginBottom: 12, textAlign: "center" },
    paywallSubtitle: { fontSize: 15, color: c.textPrimary, textAlign: "center", lineHeight: 22, marginBottom: 20 },
    paywallDivider: { width: 48, height: 3, backgroundColor: c.accent, borderRadius: 2, marginBottom: 20 },
    paywallPerks: { fontSize: 14, fontWeight: "700", color: c.accentText, marginBottom: 10 },
    paywallPerksList: { alignSelf: "stretch", gap: 8, marginBottom: 32 },
    paywallPerk: { fontSize: 14, color: c.textPrimary, textAlign: "center" },
    paywallButton: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: c.accent, paddingHorizontal: 28, paddingVertical: 14, borderRadius: 50, shadowColor: c.accent, shadowOpacity: 0.4, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } },
    paywallButtonText: { fontSize: 16, fontWeight: "700", color: "#fff" },
  });
}
